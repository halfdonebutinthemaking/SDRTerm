...
\begin{axis}[
  ...
  % Noise floor with adjacent channel peaks
  \addplot[thick, blue!70!black, domain=-1000:1000, samples=400]
    { -50 + rand*4
      + 40 * exp(-((x-100)/8)^2)      % our burst at +100 kHz
      + 20 * exp(-((x+42)/6)^2)       % adjacent chan -42
      + 20 * exp(-((x-142)/6)^2)      % adjacent chan +142
      + 15 * exp(-((x-542)/8)^2)      % far interferer
      + 12 * exp(-((x+700)/10)^2) };  % far interferer
...
\begin{axis}[
  ...
  % Only content within ±20 kHz survives
  \addplot[thick, blue!70!black, domain=-1000:1000, samples=400]
    { -60 + rand
      + 40 * exp(-((x-100)/8)^2)
      - 50 * ((abs(x)>50) ? 1 : 0) };
